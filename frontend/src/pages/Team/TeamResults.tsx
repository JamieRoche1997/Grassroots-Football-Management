...
export default function TeamResults() {
  ...
  const fetchPreviousMatches = useCallback(async () => {
    ...
    try {
      ...
      const monthPromises = Array.from({ length: currentMonth }, (_, i) => i + 1)
        .map(month => {
          const formattedMonth = `${currentYear}-${month.toString().padStart(2, '0')}`;
          return fetchFixturesByMonth(formattedMonth, clubName, ageGroup, division);
        });

      const monthlyMatches = await Promise.all(monthPromises);
      allMatches = monthlyMatches.flat();

      const pastMatches = allMatches.filter(match => parseISO(match.date) < new Date());

      const enrichedMatches = await Promise.all(
        pastMatches.map(async (match) => {
          try {
            const result = await getResult(match.matchId, clubName, ageGroup, division);
            return { ...match, ...result };
          } catch {
            return match;
          }
      ...
      setMatches(enrichedMatches.sort((a, b) => 
            ...
            {matches.map((match) => {
              ...
              return (
                <Grid size={{ xs: 12, sm: 6, md: 4}} key={match.matchId}>
                  <MatchCard
                    onClick={isCoach ? () => navigate(`/team/results/${match.matchId}`, { state: { match } }) : undefined}
                  >
                      ...
                      <Stack spacing={2}>
                        ...
                        <Stack direction="row" spacing={1} alignItems="center" justifyContent="center">
                          ...
                          <Typography variant="body2" color="text.secondary">
                            {format(parseISO(match.date), 'MMMM d, yyyy • h:mm a')}
                          </Typography>
                        </Stack>
                      </Stack>
                    </CardContent>
