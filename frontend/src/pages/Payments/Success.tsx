...
export default function Success() {
    ...
    const { transactions, loading, error } = useTransactions(user?.email ?? undefined);
    ...
    return (
        <Layout>
            ...
            <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", mt: 5, textAlign: "center" }}>
                <Typography variant="h4" sx={{ mb: 3 }}>🎉 Payment Successful!</Typography>
                <Typography variant="body1" sx={{ mb: 3, maxWidth: "500px" }}>
                    Your payment was processed successfully. Below is a summary of your purchase.
                </Typography>
                ...
                {loading ? (
                    ...
                    <Paper sx={{ p: 3, width: "100%", maxWidth: 500, textAlign: "left" }}>
                        <Typography variant="h6" sx={{ mb: 1 }}>🛍️ Order Summary</Typography>
                        <Divider sx={{ mb: 2 }} />
                        ...
                        <Divider sx={{ my: 2 }} />
                        <Typography variant="h6">Total: €{latestTransaction.amount.toFixed(2)}</Typography>
                    </Paper>
                ) : (
                    <Typography variant="body1" sx={{ my: 2 }}>No recent transaction found.</Typography>
                )}

                <Button variant="contained" sx={{ mt: 3 }} onClick={() => navigate("/payments/shop")}>
                    Return to Shop
                </Button>
            </Box>
        </Layout>
