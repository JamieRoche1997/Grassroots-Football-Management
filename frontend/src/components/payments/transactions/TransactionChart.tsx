    ...
    <ResponsiveContainer width="100%" height={300}>
      <PieChart>
        <Pie
          ...
          innerRadius={60} 
          fill="#8884d8"
          label={({ name, percent }) => `${Capitalise(name)} ${(percent * 100).toFixed(0)}%`} // ✅ Capitalised label
        >
          {data.map((_, index) => (
        ...
        <Tooltip
          formatter={(value: number, name: string) => [`€${value.toFixed(2)}`, Capitalise(name)]} // ✅ Capitalise & add €
        />
        <Legend formatter={(value) => Capitalise(value)} /> {/* ✅ Capitalised legend labels */}
      </PieChart>
    </ResponsiveContainer>
  );
}
