 ...
 * @param role - The user's role.
 */
export const signUpWithGoogle = async (role: string): Promise<void> => {
  try {
...
export const verifyIdToken = async (
  ...
  try {
    ...
    await updateProfile(verifiedData.email, {
...
export const checkUserExists = async (email: string): Promise<boolean> => {
  ...
  if (response.status === 404) {
    ...
    console.error(
      `checkUserExists failed for ${email}:`,
...
export const createUserPre = async (
  email: string,
  uid: string,
  role: string
): Promise<void> => {
